...
void L1PhaseIIObjMaker::run(const edm::Event &ev)
{
  ...
  if ( !theGmtDataToken.isUninitialized())  makeGmtCandidates(ev, L1PhaseIIObj::uGMT    , theL1Objs);
  if ( !theGmtEmulToken.isUninitialized())  makeGmtCandidates(ev, L1PhaseIIObj::uGMT_emu, theL1Objs);
  if (!theOmtfDataToken.isUninitialized())  makeRegCandidates(ev, L1PhaseIIObj::OMTF    , theL1Objs);
  if (!theOmtfEmulToken.isUninitialized())  makeRegCandidates(ev, L1PhaseIIObj::OMTF_emu, theL1Objs);
  if (!theBmtfDataToken.isUninitialized())  makeRegCandidates(ev, L1PhaseIIObj::BMTF    , theL1Objs);
  if (!theEmtfDataToken.isUninitialized())  makeRegCandidates(ev, L1PhaseIIObj::EMTF    , theL1Objs);

/*
  if (theConfig.exists("l1RpcSource"))     getGMTReadout( ev, theL1Objs, theConfig.getParameter<edm::InputTag>("l1RpcSource"),    L1PhaseIIObj::RPCb);
  if (theConfig.exists("l1RpcSource"))     getGMTReadout( ev, theL1Objs, theConfig.getParameter<edm::InputTag>("l1RpcSource"),    L1PhaseIIObj::RPCf);
  if (theConfig.exists("l1CscSource"))     getGMTReadout( ev, theL1Objs, theConfig.getParameter<edm::InputTag>("l1CscSource"),    L1PhaseIIObj::CSC);
  if (theConfig.exists("l1DtSource") )     getGMTReadout( ev, theL1Objs, theConfig.getParameter<edm::InputTag>("l1DtSource"),     L1PhaseIIObj::DT);
  if (theConfig.exists("l1GmtSource"))     getGMTReadout( ev, theL1Objs, theConfig.getParameter<edm::InputTag>("l1GmtSource"),    L1PhaseIIObj::GMT);
  if (theConfig.exists("l1RpcEmuSource"))  getRpcRegional(ev, theL1Objs, theConfig.getParameter<edm::InputTag>("l1RpcEmuSource"));
  if (theConfig.exists("l1GmtEmuSource"))  getGMTReadout( ev, theL1Objs, theConfig.getParameter<edm::InputTag>("l1GmtEmuSource"), L1PhaseIIObj::GMT_emu);
*/
...
bool L1PhaseIIObjMaker::makeRegCandidates(const edm::Event &iEvent,  L1PhaseIIObj::TYPE type, std::vector<L1PhaseIIObj> &result)
{
  ...
  //int bxNumber = 0;
  //if (bxNumber!=0 && type==L1PhaseIIObj::OMTF_emu) continue;
...
/*
bool L1PhaseIIObjMaker::getGMTReadout(const edm::Event &ev, vector<L1PhaseIIObj> &result, const edm::InputTag &readout, L1PhaseIIObj::TYPE type)
{
  ...
  for( RRItr = gmt_records.begin() ; RRItr != gmt_records.end() ; RRItr++ ) {
    ...
    switch (type) {
      case L1PhaseIIObj::RPCb: { cands = RRItr->getBrlRPCCands(); break; }
      case L1PhaseIIObj::RPCf: { cands = RRItr->getFwdRPCCands(); break; }
      case L1PhaseIIObj::DT:   { cands = RRItr->getDTBXCands(); break; }
      case L1PhaseIIObj::CSC:  { cands = RRItr->getCSCCands(); break; }
      case L1PhaseIIObj::GMT:      { gmts  = RRItr->getGMTCands(); break; }
      case L1PhaseIIObj::GMT_emu:  { gmts  = RRItr->getGMTCands(); break; }
      default: break;
    };
