...
void RPCLinkSynchroHistoMaker::fill(TH1F* hDelay, TH2F* hDelaySpread, TH2F* hTopOccup, TH2F* hTopSpread, TH2F* hTopDelay) const
{
  hDelay->Reset();
  hDelaySpread->Reset();
  hTopOccup->Reset();
  ...
  typedef std::vector< std::pair<double, unsigned int> > TopDelay;
  TopOccup topOccup(10,std::make_pair(0,0));  
  TopSpread topSpread(10,std::make_pair(0.,0));
  TopDelay  topDelay(10,std::make_pair(0.,0));
  ...
  for (unsigned int idx=0; idx < theLinkStat.theLinkStatMap.size(); ++idx) {
    ...
    double delay = fabs(bc.second.mean()-3.);
    ...
    std::pair<double, unsigned int>       canDelay =  std::make_pair(delay, idx);
    ...
    TopDelay::iterator id = upper_bound(topDelay.begin(), topDelay.end(), canDelay, OrderLbSpread()); 
    if (io != topOccup.end()) { 
    ...
    if (id != topDelay.end()) {
      topDelay.insert(id,canDelay);
      topDelay.erase(topDelay.end()-1);
    }
  }
  ...
  for (int itop=0; itop<10; itop++) {
    ...
    const RPCLinkSynchroStat::BoardAndCounts & delay  = theLinkStat.theLinkStatMap[topDelay[itop].second];
    ...
    hTopDelay->GetYaxis()->SetBinLabel(itop+1,delay.first.name().c_str());
    for (unsigned int icount=0; icount<occup.second.counts().size(); icount++) {
      ...
      hTopDelay->SetBinContent(icount+1, itop+1, float(delay.second.counts()[icount]));
    }
  }
//  for (int j=0; j<10; j++) { cout <<"topSpread["<<j<<"] = "<<topSpread[j].first<<endl; }
//  for (int j=0; j<10; j++) { cout <<"topOccup["<<j<<"] = "<<topOccup[j].first<<endl; }
//  for (int j=0; j<10; j++) { std::cout <<"topDelay["<<j<<"] = "<<topDelay[j].first<<std::endl; }
