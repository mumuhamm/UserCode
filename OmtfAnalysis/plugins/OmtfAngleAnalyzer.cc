...
int eta_bit(float eta) { return bounds.rend() - std::lower_bound (bounds.rbegin(), bounds.rend(), eta); }
...
void OmtfAngleAnalyzer::beginRun(const edm::Run&,  const edm::EventSetup& es)
 {
  ...
  for (auto me : lbrolls) {
    ...
    for (unsigned int lbChannel = 0; lbChannel <= 95; lbChannel++) {
      ...
      if (rawDetId && strip) {
        ...
        unsigned int eta_scale = 1 << ::eta_bit( fabs(eta_value) );
...
std::string OmtfAngleAnalyzer::lbNameforDetId(const RPCReadOutMapping* cabling, uint32_t rawDetId)
{
